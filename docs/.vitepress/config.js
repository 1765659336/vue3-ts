export default {
    title: '胖子Fatty',
    description: '',
    lastUpdated: true,
    markdown: {
        theme: 'material-palenight',
        lineNumbers: true,
    },
    themeConfig: {
        setTitle: false,
        logo: '/logo.ico',
        lastUpdatedText: "最后更新时间",
        nav: [
            { text: '首页Home', link: '/' },
            {
                text: '文档Document', items: [
                    { text: '玩具组件Custom', link: '/Custom/index' },
                    { text: '业务组件SecondaryPackaging', link: '/SecondaryPackaging/index' },
                    { text: '自定义指令VInstructions', link: '/VInstructions/index' },
                    { text: '动画Animation', link: '/Animation/index' },
                ]
            },
            { text: '团队Team', link: '/Team/index' },
            {
                text: '讨论Discuss',
                items: [
                    { text: 'QQ群', link: 'https://jq.qq.com/?_wv=1027&k=Zhdmd0xz' },
                ]
            }
        ],
        socialLinks: [
            { icon: 'github', link: 'https://github.com/1765659336/vue3-ts' },
            { icon: { svg: '<svg height="32" viewBox="0 0 32 32" width="32" xmlns="http://www.w3.org/2000/svg"><g fill="none" fill-rule="evenodd"><circle cx="16" cy="16" fill="#c71d23" r="16"/><path d="m24.0987698 14.2225144h-9.0863697c-.4362899.000207-.7900048.3538292-.790326.7901191l-.0005173 1.9752185c-.0003277.4363707.353328.7902117.7896987.790326.0000712 0 .0001424 0 .0002135-.0002135l5.5317648-.0000461c.4363708-.0000102.7901221.3537352.7901257.790106 0 .0000022 0 .0000044-.0000066.0000066v.1975077.1975318c0 1.3091122-1.0612451 2.3703573-2.3703573 2.3703573h-7.5067195c-.4363081-.0000218-.790009-.353713-.7900429-.7900211l-.0002069-7.5059917c-.0001014-1.3091122 1.0611145-2.3703865 2.3702267-2.3704226.0000217 0 .0000435 0 .0000653.0000653h11.0602463c.4361793-.0004902.7898484-.35394.7906091-.79011894l.0012251-1.97521881c.0007606-.43637034-.3527683-.79033806-.7891389-.79060871-.0001634-.0000001-.0003268-.00000015-.0004901.00048976h-11.0617654c-3.27278051 0-5.92589329 2.65311278-5.92589329 5.9258933v11.0612755c0 .4363707.35374837.7901191.7901191.7901191h11.65447149c2.9454379 0 5.3331872-2.3877493 5.3331872-5.3331872v-4.5430682c0-.4363707-.3537484-.7901191-.7901191-.7901191z" fill="#fff"/></g></svg>' }, link: 'https://gitee.com/ctbaobao/vue3-ts-ui' },
            { icon: { svg: '<svg width="36" height="28" viewBox="0 0 36 28" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M17.5875 6.77268L21.8232 3.40505L17.5875 0.00748237L17.5837 0L13.3555 3.39757L17.5837 6.76894L17.5875 6.77268ZM17.5863 17.3955H17.59L28.5161 8.77432L25.5526 6.39453L17.59 12.6808H17.5863L17.5825 12.6845L9.61993 6.40201L6.66016 8.78181L17.5825 17.3992L17.5863 17.3955ZM17.5828 23.2891L17.5865 23.2854L32.2133 11.7456L35.1768 14.1254L28.5238 19.3752L17.5865 28L0.284376 14.3574L0 14.1291L2.95977 11.7531L17.5828 23.2891Z" fill="#1E80FF"/></svg>' }, link: 'https://juejin.cn/column/7159983255200989221' },
            { icon: { svg: '<svg width="32" height="32" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><g><title>Layer 1</title><image xlink:href="data:image/x-icon;base64," xlink:href="data:image/x-icon;base64," id="svg_1" height="32" width="32" y="0" x="0"/></g></svg>' }, link: 'https://blog.csdn.net/weixin_44242181/category_12078863.html' },
        ],
        sidebar: {
            '/Custom/': [
                {
                    text: 'Custom自定义',
                    items: [
                        { text: 'Affix固钉', link: '/Custom/Affix' },
                        { text: 'BackTop回到顶部', link: '/Custom/BackTop' },
                        { text: 'Badge徽章', link: '/Custom/Badge' },
                        { text: 'Button按钮', link: '/Custom/Button' },
                        { text: 'Cantainer布局容器', link: '/Custom/Cantainer' },
                        { text: 'Checkbox复选框', link: '/Custom/Checkbox' },
                        { text: 'Collapse折叠面板', link: '/Custom/Collapse' },
                        { text: 'Dropdown下拉菜单', link: '/Custom/Dropdown' },
                        { text: 'Link文字链接', link: '/Custom/Link' },
                        { text: 'Message消息提示框', link: '/Custom/Message' },
                        { text: 'Popover气泡弹出框', link: '/Custom/Popover' },
                        { text: 'Rate评分', link: '/Custom/Rate' },
                        { text: 'Signature绘画板', link: '/Custom/Signature' },
                        { text: 'Slider滑块', link: '/Custom/Slider' },
                        { text: 'Switch开关', link: '/Custom/Switch' },
                        { text: 'Tag标签', link: '/Custom/Tag' },
                        { text: 'Timeline时间轴', link: '/Custom/Timeline' },
                        { text: 'Transfer穿梭框', link: '/Custom/Transfer' },
                    ]
                }],
            '/SecondaryPackaging/': [
                {
                    text: 'SecondaryPackaging二次封装',
                    items: [
                        { text: 'SecondEchart', link: '/SecondaryPackaging/SecondEchart' },
                    ]
                }
            ],
            '/Animation/': [
                {
                    text: '无',
                    items: [
                    ]
                }
            ],
            '/VInstructions/': [
                {
                    text: '无',
                    items: [
                    ]
                }
            ]
        },
        footer: {
            message: 'salute-向你敬礼',
            copyright: '<a href="https://portrait.gitee.com/uploads/avatars/user/2951/8855929_ctbaobao_1661007073.png!avatar200" target="view_window">Show off Girlfriend</a>',
        }
    },
}